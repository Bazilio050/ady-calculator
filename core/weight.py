...
def get_min_loading_norm(gng_code: str) -> int:
    ...
    # 1. Зерновые культуры (Группа 10: 1001, 1002, 1003, 1004, 1005 и т.д.) — 60 тонн
    if clean_gng.startswith("10") or clean_gng.startswith("010"):
        return 60
    ...
    # 4. Минеральные удобрения (Группа 31, кроме органических 3101) — 60 тонн
    if (clean_gng.startswith("31") or clean_gng.startswith("031")) and not clean_gng.startswith("3101"):
        return 60

    # 5. Уголь, Руда, Чугун, Сахар, Мука, Зернопродукты (2701, 2702, 7201, 1701, 1101-1107) — 60 тонн
    if any(clean_gng.startswith(p) for p in [
        "2701", "2702", "7201", "1701", "1101", "1102", "1103", "1104", "1105", "1106", "1107"
    ]):
        return 60

    # 6. Хлопок, Лом черных металлов (14042, 5201-5203, 7204) — 50 тонн
