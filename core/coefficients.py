...
def get_applicable_coefficients(
    ...
    # ------------------------------------------------------------------------------
    # 9-11. ПРАВИЛО "BIR DƏFƏ": Коэффициент 1.20 применяется ровно 1 раз
    # ------------------------------------------------------------------------------
    has_120_applied = False

    # (A) Транзит нефти и нефтепродуктов (Таблица 6) — "bir dəfə"
    if (is_import or is_transit) and table_str == "6":
        add_coeff("oil_products", 1.20)
        has_120_applied = True

    # (B) Транзит рефрижераторов (Таблица 5) — "bir dəfə"
