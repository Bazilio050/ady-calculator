...
def get_table_6_column(gng_code: str, is_private_tank: bool = False) -> int:
    ...
    if not code:
        return 7  # По умолчанию прочие грузы

    # Колонка 8: Углеводороды в частных цистернах
    ...
    # Колонка 2: Нефть и нефтепродукты
    ...
    # Колонка 3: Энергетические газы
    if code.startswith(("2705", "2711")):
        return 3

    # Колонка 4: Газы и углеводороды
...
def select_tariff_table(wagon_category: str, shipment_type: str, is_empty_inventory: bool = False, gng_code: str = "") -> dict:
    """
    ...
    # Наливные грузы (цистерны, бункерные полувагоны)
