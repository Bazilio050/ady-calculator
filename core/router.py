# ------------------------------------------------------------------------------
# БЛОК 1: Модуль маршрутизации и определения типа перевозки ADY
# ------------------------------------------------------------------------------
from dataclasses import dataclass
from enum import Enum
from typing import Optional
from data.stations_mapping import STATIONS_MAPPING, get_canonical_station_name, get_station_code
...
class RailwayRouter:
    # Список ключевых пограничных / экспортных станций ADY
    BORDER_KEYWORDS = [
    ...
    def _is_border_station(self, canonical_name: str, code: str) -> bool:
        # Проверка по наименованию и кодам погранпереходов
    ...
    def _get_distance_from_file(self, from_code: str, to_code: str) -> float:
        # Прямой поиск расстояния по файлу Distances.txt
        with open(self.distances_file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()

        # Поиск строки по коду станции отправления или назначения
        for line in lines:
            parts = [p.strip() for p in line.split("|") if p.strip()]
            if len(parts) >= 7 and parts[1] == from_code:
                # В зависимости от пограничного узла назначения выбираем столбец
                # Yalama=2, Astara=3, Boyuk Kesik=4, Culfa=5, Alat=6
        ...
        # Если прямое расстояние между парами станций зафиксировано — считываем его
    ...
    def calculate_route(self, raw_from: str, raw_to: str) -> RouteResult:
        ...
        try:
            ...
            # Симметричная проверка в обратную сторону
