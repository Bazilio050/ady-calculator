...
# ------------------------------------------------------------------------------
# ИНТЕГРАЦИОННЫЕ ТЕСТЫ ДЛЯ ТАБЛИЦЫ 5 (СПС)
# ------------------------------------------------------------------------------
def test_refrigerator_import_calculation():
    """Тест 9: Импорт в рефрижераторе (Ялама -> Апшерон) | Реф. вагон | 28т | СПС"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Ялама", "Апшерон")
    effective_dist = route_res.calculated_distance_km if route_res.calculated_distance_km > 0 else route_res.distance_km
    ...
    _print_calculation_result("9. Импорт в рефрижераторе (Ялама -> Апшерон)", route_res, calc_res, "0404", 28, "refrigerator", True)
    assert calc_res["base_rate_chf_per_ton"] > 0


def test_thermos_transit_calculation():
    """Тест 10: Транзит в термосе (Ялама -> Беюк Кясик) | Термос | 22т | СПС"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Ялама", "Беюк Кясик")
    ...
    _print_calculation_result("10. Транзит в термосе (Ялама -> Беюк Кясик)", route_res, calc_res, "0709", 22, "thermos", True)
    assert calc_res["base_rate_chf_per_ton"] > 0


def test_car_carrier_export_calculation():
    """Тест 11: Экспорт автовозом (Апшерон -> Ялама) | Автовоз | 12т | СПС"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Апшерон", "Ялама")
    effective_dist = route_res.calculated_distance_km if route_res.calculated_distance_km > 0 else route_res.distance_km
    ...
    _print_calculation_result("11. Экспорт автовозом (Апшерон -> Ялама)", route_res, calc_res, "8703", 12, "car_carrier", True)
    assert calc_res["base_rate_chf_per_ton"] > 0


def test_inv_anv_empty_calculation():
    """Тест 12: Порожний проход ИНВ/АНВ (Ялама -> Беюк Кясик) | ИНВ/АНВ | 0т | Порожний"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Ялама", "Беюк Кясик")
    ...
    _print_calculation_result("12. Порожний проход ИНВ/АНВ (Ялама -> Беюк Кясик)", route_res, calc_res, "8606", 0, "inv_anv", False)
    assert calc_res["base_rate_chf_per_ton"] > 0
