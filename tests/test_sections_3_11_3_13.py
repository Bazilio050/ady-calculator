# tests/test_sections_3_11_3_13.py
...
def test_coffin_transport_rule_3_11():
    """Тест 1: Перевозка гробов с телами усопших (п. 3.11) — коэффициент 0.10"""
    ...
    calc_res = TariffCalculator.calculate(
        ...
        wagon_type="covered",
        is_coffin_transport=True,
        from_canonical_name=route_res.from_station.canonical_name,
        to_canonical_name=route_res.to_station.canonical_name,
        is_private_wagon=True
    )

    _print_calculation_result("Запуск тестов Правила 3.11 (Перевозка гробов, coeff 0.10)", route_res, calc_res, "10010000", 10.0, "covered", True)

    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", []) if isinstance(n, dict)]
    assert "COFFIN_TRANSPORT_COEFF_0_10_RULE_3_11" in applied_codes


def test_separate_locomotive_rule_3_12():
    """Тест 2: Перевозка с отдельным локомотивом (п. 3.12) — коэффициент 5.00"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Ялама", "Беюк Кясик")
    ...
    calc_res = TariffCalculator.calculate(
        ...
        distance_km=route_res.calculated_distance_km,
        wagon_type="covered",
        is_separate_locomotive=True,
        separate_locomotive_coeff=5.00,
        from_canonical_name=route_res.from_station.canonical_name,
        to_canonical_name=route_res.to_station.canonical_name,
        is_private_wagon=True
    )

    _print_calculation_result("Запуск тестов Правила 3.12 (Отдельный локомотив, coeff 5.00)", route_res, calc_res, "10010000", 50.0, "covered", True)

    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", []) if isinstance(n, dict)]
    assert "SEPARATE_LOCOMOTIVE_COEFF_5_00_RULE_3_12" in applied_codes


def test_expedited_delivery_freight_train_rule_3_13():
    """Тест 3: Сокращенный срок доставки в грузовом поезде (п. 3.13) — коэффициент 1.50"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Ялама", "Беюк Кясик")
    ...
    calc_res = TariffCalculator.calculate(
        ...
        distance_km=route_res.calculated_distance_km,
        wagon_type="covered",
        expedited_delivery_train_type="freight",
        from_canonical_name=route_res.from_station.canonical_name,
        to_canonical_name=route_res.to_station.canonical_name,
        is_private_wagon=True
    )

    _print_calculation_result("Запуск тестов Правила 3.13 (Ускоренная доставка в грузовом поезде, coeff 1.50)", route_res, calc_res, "10010000", 50.0, "covered", True)

    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", []) if isinstance(n, dict)]
    assert "EXPEDITED_DELIVERY_FREIGHT_TRAIN_COEFF_1_50_RULE_3_13" in applied_codes
