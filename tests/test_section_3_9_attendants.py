# tests/test_section_3_9_attendants.py
...
def test_attendants_basic_rule_3_9():
    """Тест 1: Проезд 2 проводников с грузом (п. 3.9) — 12.00 CHF / 100 км"""
    ...
    _print_calculation_result("Запуск тестов Правила 3.9 (Проезд 2 проводников)", route_res, calc_res, "10010000", 50.0, "covered", True)

    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", []) if isinstance(n, dict)]
    assert "ATTENDANTS_FEE_RULE_3_9" in applied_codes


def test_separate_attendant_wagon_private_rule_3_9():
    """Тест 2: Проезд проводников в отдельном приватном грузовом вагоне / теплушке (п. 3.9, 0.20 CHF/ось-км)"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    route_res = router.calculate_route("Ялама", "Беюк Кясик")
    ...
    calc_res = TariffCalculator.calculate(
        ...
        wagon_type="covered",
        attendants_count=2,
        is_separate_attendant_wagon=True,
        axle_count=4,
        from_canonical_name=route_res.from_station.canonical_name,
        to_canonical_name=route_res.to_station.canonical_name,
        is_private_wagon=True
    ...
    _print_calculation_result("Запуск тестов Правила 3.9 (Отдельная приватная теплушка 0.20 CHF/ось-км)", route_res, calc_res, "99200000", 0.0, "covered", True)

    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", []) if isinstance(n, dict)]
    assert "SEPARATE_ATTENDANT_WAGON_RULE_3_9" in applied_codes
