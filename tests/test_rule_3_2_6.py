...
# ------------------------------------------------------------------------------
# БЛОК 3: Тестовые сценарии для Пункта 3.2.6 (Автопоезда на спецплатформах)
# ------------------------------------------------------------------------------

def test_rule_3_2_6_road_train_import():
    """Тест 1: Груженый автопоезд на приватной спецплатформе по п. 3.2.6 — Импорт (Ялама -> Абшерон)"""
    ...
    _print_calculation_result(
        "1. Перевозка автопоезда по п. 3.2.6 [Импорт: Ялама -> Абшерон]",
        route_res, calc_res, "87163900", 20.0, "road_train (Спецплатформа, приватный)"
    )
    ...
    assert "ROAD_TRAIN_SPECIAL_PLATFORM_RULE_3_2_6" in applied_codes
    assert "MAIN_COEFF_0_85_PRIVATE_WAGON" in applied_codes


def test_rule_3_2_6_semi_trailer_export():
    """Тест 2: Порожний полуприцеп на приватной спецплатформе по п. 3.2.6 — Экспорт (Гянджа -> Ялама)"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    ...
    _print_calculation_result(
        "2. Порожний полуприцеп по п. 3.2.6 [Экспорт: Гянджа -> Ялама]",
        route_res, calc_res, "99220000", 0.0, "semi_trailer (Спецплатформа, приватный порожний)"
    )
    assert calc_res["base_rate_chf_per_ton"] > 0
    assert calc_res["applied_table"] == "Таблица 5"
    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", [])]
...
def test_rule_3_2_6_road_train_transit():
    """Тест 3: Груженый автопоезд на приватной спецплатформе по п. 3.2.6 — Транзит (Ялама -> Беюк Кясик)"""
    router = RailwayRouter(distances_file_path="data/distances.csv")
    ...
    calc_res = TariffCalculator.calculate(
        ...
        actual_weight=22.0,
        distance_km=effective_dist,
        wagon_type="road_train_platform",
        from_canonical_name=route_res.from_station.canonical_name,
        to_canonical_name=route_res.to_station.canonical_name,
        is_private_wagon=True,
        is_empty_wagon=False
    ...
    _print_calculation_result(
        "3. Перевозка автопоезда по п. 3.2.6 [Транзит: Ялама -> Беюк Кясик]",
        route_res, calc_res, "87163900", 22.0, "road_train_platform (Спецплатформа, приватный)"
    )
    assert calc_res["base_rate_chf_per_ton"] > 0
    assert calc_res["applied_table"] == "Таблица 5"
    applied_codes = [n.get("rule_code") for n in calc_res.get("notifications", [])]
