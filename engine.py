...
def process_full_calculation(nlu_data: dict, user_input_raw: str = "", lang: str = "AZ", year: str = "2026", ui_t: dict = None, *args, **kwargs) -> dict:
    ...
    # Мин. плечо 151 км для химических цистерн п. 3.2.5
    ...
    # Нормы веса по разделу 3.3 и п. 3.8 (yığma - min 10t)
    ...
    if is_cover_wagon:
        ...
        r_val, _ = calculate_table_4_base(tariff_dist_km, bw) if shipment_type_code == "transit" else calculate_table_3_base(tariff_dist_km, bw)
    ...
    # 2. Скидка парка SPS (0.85 или 0.70 по п. 3.2.5)
    ...
    # 3. Базовый коэффициент 1.50 (Импорт / Экспорт) и исключения
    ...
    is_alat_bk = ("alat" in st_from_raw.lower() and "boyuk" in st_to_raw.lower()) or ("boyuk" in st_from_raw.lower() and "alat" in st_to_raw.lower()) or ("kuryk" in input_lower and "boyuk" in st_to_raw.lower()) or ("trk" in input_lower and "boyuk" in st_to_raw.lower())
    ...
    if wagon_len_req and (bool(nlu_data.get("is_asco_ferry")) or "паром" in input_lower or "bərə" in input_lower):
        ...
        # Регулировка длины цистерн (13м)
        if wagon_type in ["cistern", "цистерна", "çən"] and w_len <= 15.0:
            w_len = 13.0

        is_gas = "gas" in input_lower or "газ" in input_lower or clean_gng.startswith("2705") or clean_gng.startswith("2711")
